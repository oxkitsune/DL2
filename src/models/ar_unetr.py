...
class AR_UNETR(nn.Module):
    def __init__(
        ...
        # series of convolutions to reduce depth from 128 to 8 for autoregression 
        # NOTE: we can make axis with stride 2 a hyperparameter, to dynamically change which axis to autoregress
        self.conv1 = nn.Conv3d(output_dim, output_dim, kernel_size=3, stride=(1, 1, 2), padding=1)  # Reduce depth from 128 to 64
        self.conv2 = nn.Conv3d(output_dim, output_dim, kernel_size=3, stride=(1, 1, 2), padding=1)  # Reduce depth from 64 to 32
        self.conv3 = nn.Conv3d(output_dim, output_dim, kernel_size=3, stride=(1, 1, 2), padding=1)  # Reduce depth from 32 to 16
        self.conv4 = nn.Conv3d(output_dim, output_dim, kernel_size=3, stride=(1, 1, 2), padding=1)  # Reduce depth from 16 to 8
